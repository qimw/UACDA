...
def get_arguments():
    ...
    parser.add_argument("--kl-warm-up", type=int, default=KL_WARM_UP,
                        help="KL distance warm up.")
...
def main():
    ...
    # set up tensor board
    if args.tensorboard:
        args.log_dir = os.path.join(args.log_dir, args.snapshot_dir.split('/')[-2], args.snapshot_dir.split('/')[-1])
        if not os.path.exists(args.log_dir):
            os.makedirs(args.log_dir)

        writer = SummaryWriter(args.log_dir)
    start_iter = 0
    # if 'http' not in args.restore_from:
    #     base_name = os.path.basename(args.restore_from) 
    #     start_iter = int(base_name.split('.')[0].split('_')[-1])   
    ...
    for i_iter in range(start_iter, args.num_steps):
        ...
        eta = (current_time - start_time) / (i_iter - start_iter + 1) * (args.num_steps - i_iter) / 3600
        print('exp = {}'.format(args.snapshot_dir))
        print(
        '\033[1m iter = %8d/%8d \033[0m loss_seg1 = %.3f loss_seg2 = %.3f loss_me = %.3f  loss_kl = %.3f loss_adv1 = %.3f, loss_adv2 = %.3f loss_D1 = %.3f loss_D2 = %.3f eta = %.3f'%(i_iter, args.num_steps, loss_seg_value1, loss_seg_value2, loss_me_value, loss_kl, loss_adv_target_value1, loss_adv_target_value2, loss_D_value1, loss_D_value2, eta))

        # clear loss
        del loss_seg1, loss_seg2, loss_adv_target1, loss_adv_target2, loss_me, loss_kl


        if (i_iter + 1) % args.save_pred_every == 0:
            print('taking snapshot ...')
            torch.save(Trainer.G.state_dict(), osp.join(args.snapshot_dir, 'GTA5_' + str(i_iter+1) + '.pth'))
            torch.save(Trainer.D1.state_dict(), osp.join(args.snapshot_dir, 'GTA5_' + str(i_iter+1) + '_D1.pth'))
            torch.save(Trainer.D2.state_dict(), osp.join(args.snapshot_dir, 'GTA5_' + str(i_iter+1) + '_D2.pth'))
