...
class AD_Trainer(nn.Module):
    def __init__(self, args):
        ...
        self.kl_warm_up = args.kl_warm_up
            ...
            for i in saved_state_dict:
                ...
                if args.restore_from[:4] == 'http' :
                    ...
                    if i_parts[0] =='module':
                        ...
                        print('%s is loaded from pre-trained weight.\n'%i_parts[0:])
            # NOTE QIMENG: restore D
            if 'http' not in args.restore_from:
                print('load discriminators')
                dir_path = os.path.dirname(args.restore_from)
                base_name = os.path.basename(args.restore_from) 
                start_iter = base_name.split('.')[0].split('_')[-1]
                files = os.listdir(dir_path)
                for f in files:
                    if start_iter in f and 'D1' in f:
                        D1_path = os.path.join(dir_path, f)
                    elif start_iter in f and 'D2' in f:
                        D2_path = os.path.join(dir_path, f)
                D1_weight = torch.load(D1_path)
                D2_weight = torch.load(D2_path)
                self.D1.load_state_dict(D1_weight)
                self.D2.load_state_dict(D2_weight)
    ...
    def update_class_criterion(self, labels):
            ...
            for i in range(self.num_classes):
                ...
                if count[i]/(n*h*w) < 1/128.0: #small objective, original train size is 1024*512
